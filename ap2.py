            ...
            if st.button("🔄 저장하기"):
                try:
                    ...
                    if st.session_state.main_data is None or st.session_state.main_data.empty:
                        ...
                        st.success(f"✅ {int(current_source)}월 판매현황 데이터가 저장되었습니다!")
                    else:
                        old_df_kept = st.session_state.main_data[
                            st.session_state.main_data['업로드_출처월'] != current_source
                        ]
                        st.session_state.main_data = pd.concat([old_df_kept, new_df], ignore_index=True)
                        st.success(f"✅ {int(current_source)}월 파일 데이터가 업데이트 되었습니다!")
